...
public class DrawingEditorController {
	...
	void setupChoiceBoxes() {
		...
		drawingChoice.getSelectionModel().selectedIndexProperty().addListener((v,vOld,vNew) -> {
			...
			if (choice >= 0) {
				renderDrawing(getCurrentLabel(), choice);
			}
		});

		labelChoice.getSelectionModel().selectedIndexProperty().addListener((v, vOld, vNew) -> {
			String updatedLabel = labelChoice.getItems().get(vNew.intValue());
			renderDrawing(updatedLabel, getCurrentDrawing());
			trainer.visualizeLabel(updatedLabel);
			WritableImage img = trainer.makeVisual((int)visualization.getWidth(),(int)visualization.getHeight());
			visualization.getGraphicsContext2D().drawImage(img, 0, 0);
		});
	}

	void renderDrawing(String label, int drawing) {
		clearCanvas();
		sketch = drawings.getDrawing(label, drawing);
		if (sketch != null) {
	...
	void setupTrain() {
		...
		train.setOnAction(event -> {
			...
			startTrainingDoneThread(result, progress);
	...
	void startTrainingDoneThread(ArrayBlockingQueue<RecognizerAI> result, ArrayBlockingQueue<Double> progress) {
		new Thread(() -> {
			try {
				trainer = result.take();
				trainer.visualizeLabel(getCurrentLabel());
				WritableImage img = trainer.makeVisual((int)visualization.getWidth(),(int)visualization.getHeight());
				Platform.runLater(() -> visualization.getGraphicsContext2D().drawImage(img, 0, 0));
				Platform.runLater(() -> {info("Training finished"); System.out.println(trainer);});
			} catch (Exception e) {
				Platform.runLater(() -> oops(e));
			}
		}).start();		
	...
	void openDataFile() {
		...
		if (dataFile != null) {
			try {
				...
				resetLabels();
				clearCanvas();
				renderDrawing(getCurrentLabel(), getCurrentDrawing());
			} catch (FileNotFoundException e) {
				oops(e);
			}
		}
	...
	File getDataFile() {
		FileChooser chooser = new FileChooser();
		chooser.setTitle("Select mnist_test_10000 data");
	...
	void resetDrawingList(String label) {
		...
		if (drawings.numDrawingsFor(label) > 0) {
			drawingChoice.getSelectionModel().select(0);
		}
	}
	...
	int getCurrentDrawing() {
		Integer n = drawingChoice.getSelectionModel().getSelectedItem();
		return n == null ? 0 : n;
	}
