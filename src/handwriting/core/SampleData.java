...
public class SampleData {
	...
	public ArrayList<Duple<String,Drawing>> allSamples() {
		ArrayList<Duple<String,Drawing>> result = new ArrayList<>();
		for (String label: labelToDrawing.keySet()) {
			for (Drawing d: labelToDrawing.get(label)) {
				result.add(new Duple<>(label, d));
			}
		}
		return result;
	}
	...
	public Drawing getDrawing(String label, int index) {
		if (labelToDrawing.get(label) == null) {
			return null;
		} else {
