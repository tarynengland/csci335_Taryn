...
public class AutoCheckers extends JFrame {
    ...
    private class RunThread extends Thread {
        ...
        private Searcher player1, player2;
        ...
        private void runGame(Checkerboard board, PlayerColor player1Color) {
            ...
            while (!board.gameOver() && turn < maxTurns) {
                if (board.isTurnFor(player1Color)) {
                    board.move(player1.selectMove(board).getMove());
                    player1Nodes += player1.numNodesExpanded();
                } else {
                    board.move(player2.selectMove(board).getMove());
    ...
    private Searcher loadAlgorithms(String searchStr, String evalStr, JTextField depthField) {
        Searcher searchAlg = Searcher.makeSearcher(searchStr, evalStr, evalFuncClasses, searcherClasses);
        if (searchAlg == null) {
